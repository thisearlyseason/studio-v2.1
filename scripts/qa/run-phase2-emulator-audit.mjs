...
async function runMediaWorkflowAudit() {
  ...
  await browserStep(adultSession,'qa-adult-player-a',['media-player-self'],`${openPack}const pending=page.waitForResponse(response=>response.url().startsWith(${JSON.stringify(BASE_URL+'/api/media?')})&&response.request().method()==='POST',{timeout:15000});await dialog.locator('input[type=file]').first().setInputFiles(${JSON.stringify(imageFile)});if((await pending).status()!==201)throw Error('Player avatar upload failed');await page.getByText('Avatar Updated ✓',{exact:true}).waitFor({timeout:15000});await page.reload();await dismiss(page);return true;`);
  ...
  await browserStep(adultSession,'qa-adult-player-a',['media-delete'],`${openPack}await dialog.getByRole('tab',{name:/Gallery/}).click();await dialog.getByAltText('Gallery 0',{exact:true}).hover();let message;page.once('dialog',prompt=>{message=prompt.message();});const pending=page.waitForResponse(response=>response.url().startsWith(${JSON.stringify(BASE_URL+'/api/media?')})&&response.request().method()==='DELETE',{timeout:15000});await dialog.getByRole('button',{name:'Delete gallery photo 1',exact:true}).click();if((await pending).status()!==200)throw Error('Gallery delete failed');if(message!=='Delete this photo from the gallery?')throw Error('Unexpected native confirmation');await page.getByText('Photo Removed',{exact:true}).waitFor({timeout:15000});await page.reload();await dismiss(page);return true;`,{nativeConfirm:true});
