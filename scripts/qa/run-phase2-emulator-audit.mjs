...
function certificationActorAliases(scenarioId) {
  ...
  const actors = {
    ...
    'chat-channel-message-unread': ['qa-coach-owner-a', 'qa-team-member', 'qa-parent-a', 'qa-adult-player-a', 'qa-removed-member', 'qa-coach-owner-b', 'qa-multi-org'],
    ...
    await registerScheduleDiscovery({
      ...
      if (scenarioId === 'chat-channel-message-unread' && runBrowser) {
        ...
        for(const [dimension,caseIds] of Object.entries(LOCAL_OPERATIONS_CASE_REQUIREMENTS[scenarioId])) for(const caseId of caseIds) {
          ...
          recordObservedOperationNamedCase(scenarioId,dimension,caseId,`${caseId} completed with exact actors and owned state`,[new RegExp(`^Chat ${caseId}:`)],{
            actor:[...new Set(requests.map(request=>request.actorAlias))].sort().join('+'),
            operation:'visible tactical Chat interaction or authenticated Chat application request',requests,
            reconciliation:'exact channel/message/unread/revocation state and owned channel cleanup',timeBound:'15s UI and 20s HTTP deadlines',
          });
        }
        return;
      }
      ...
      if (scenarioId === 'practice-practice-plans-templates' && runBrowser) {
        await runPracticePlanWorkflowAudit();
        ...
        recordObservedOperationNamedCase(scenarioId, 'permission', 'plan-entitlement', 'Paid Team A sees the planner while free Team B sees the upgrade boundary', [/Practice paid and free entitlement surfaces follow team policy/], { actor: 'qa-coach-owner-a+qa-coach-owner-b', operation: 'visible Practice entitlement views', requests: operationRequestEvidence('plan-entitlement'), reconciliation: 'Team A planner visible and Team B upgrade gate visible', timeBound: '15s UI waits' });
        recordObservedOperationNamedCase(scenarioId, 'persistence', 'plan-persistence', 'Edited template and assignment survive reload', [/Practice template and assignment persist after browser reload/], { actor: 'qa-coach-owner-a+qa-team-member', operation: 'browser reload and emulator reconciliation', requests: operationRequestEvidence('plan-persistence'), reconciliation: 'exact template/event IDs retained', timeBound: '15s UI waits' });
        recordObservedOperationNamedCase(scenarioId, 'console', 'plan-console', 'Practice template workflow has no browser console errors', [/Practice template workflow console errors/], { actor: 'qa-coach-owner-a+qa-team-member+qa-coach-owner-b', operation: 'browser Practice workflow', requests: operationRequestEvidence('plan-console'), reconciliation: 'zero console errors', timeBound: 'scenario duration' });
        recordObservedOperationNamedCase(scenarioId, 'network', 'plan-network', 'Practice template workflow has no local server failures', [/Practice template workflow failed responses/], { actor: 'qa-coach-owner-a+qa-team-member+qa-coach-owner-b', operation: 'browser Practice workflow', requests: operationRequestEvidence('plan-network'), reconciliation: 'zero unallowlisted 5xx responses', timeBound: 'scenario duration' });
        recordObservedOperationNamedCase(scenarioId, 'responsive', 'plan-responsive', 'Planner and assignment surfaces fit desktop and mobile viewports', [/Practice planner and template dialog fit desktop and mobile viewports/], { actor: 'qa-coach-owner-a+qa-team-member', operation: 'visible planner and assigned event at two viewports', requests: operationRequestEvidence('plan-responsive'), reconciliation: 'document and dialog bounds inside both viewports', timeBound: 'post-workflow viewport check' });
        return;
      }
      if (scenarioId === 'practice-drill-playbook-crud-search' && runBrowser) {
        await runPracticeDrillWorkflowAudit();
        for (const [dimension, caseId, label, actor, reconciliation] of [
          ['happyPath','drill-crud','Practice drill create edit reload and delete completes','qa-coach-owner-a','one run-scoped drill lifecycle'],
          ['happyPath','drill-reorder','Practice drill reorder persists exact order after reload','qa-coach-owner-a','exact order values and rendered order'],
          ['happyPath','drill-search','Practice drill search handles exact partial empty and special-character queries','qa-coach-owner-a','exact owner-performed client-side filter result counts'],
          ['happyPath','drill-link-valid','Practice drill valid public HTTPS link persists','qa-coach-owner-a','exact stored HTTPS URL'],
          ['negativePath','drill-link-invalid','Practice drill rejects javascript credentialed private-host malformed and overlong URLs','qa-coach-owner-a','zero unsafe durable URLs'],
          ['negativePath','drill-duplicate-empty','Practice drill rejects duplicate and empty submissions','qa-coach-owner-a','zero duplicate or empty documents'],
          ['permission','drill-member-deny','Practice member drill mutation is denied','qa-team-member','403 and unchanged Team A drill'],
          ['permission','drill-team-b-deny','Practice Team B owner drill mutation and discovery are denied','qa-coach-owner-b','403 and 403 Team A boundary'],
          ['persistence','drill-persistence','Practice drill content and order persist after reload','qa-coach-owner-a','exact content and order after reload'],
          ['console','drill-console','Practice drill workflow console errors','qa-coach-owner-a+qa-team-member','zero console errors'],
          ['network','drill-network','Practice drill workflow failed responses','qa-coach-owner-a+qa-team-member','zero unallowlisted 5xx responses'],
          ['responsive','drill-responsive','Practice and drills surfaces fit desktop and mobile viewports','qa-coach-owner-a+qa-team-member','actual page and dialog bounds'],
        ]) recordObservedOperationNamedCase(scenarioId, dimension, caseId, label, [new RegExp(label)], { actor, operation: 'visible Playbook and exact Firestore requests', requests: operationRequestEvidence(caseId), reconciliation, timeBound: '15s UI waits and immediate emulator reads' });
        return;
      }
      if (scenarioId === 'practice-film-upload-coach-marks-watch' && runBrowser) {
        await runPracticeFilmWorkflowAudit();
        for (const [dimension, caseId, label, actor, reconciliation] of [
          ...
          ['permission','film-team-b','Practice film Team B cannot access Team A film','qa-coach-owner-b','403 metadata and object boundary'],
          ['persistence','film-progress-own','Practice film player own watch progress persists','qa-adult-player-a','one self-keyed progress record'],
          ['persistence','film-delete','Practice film delete removes metadata and exact object','qa-coach-owner-a','404 metadata and object reads'],
...
async function capturePracticeDocumentReads(caseId, actorTokens, documentPath) {
  const actorAliases = actorTokens.map(item => item.alias);
  return captureOperationRequests(caseId, actorAliases.join('+'), async () => {
    const statuses = [];
    for (const item of actorTokens) statuses.push(await directFirestoreReadStatus(documentPath, item.token));
    return statuses;
  });
}
...
async function runPracticePlanWorkflowAudit() {
  ...
  const invalidDrillId = `plan-invalid-drill-${certificationRunId}`;
  const drillPath = `teams/${teamA.id}/drills/${drillId}`;
  const invalidDrillPath = `teams/${teamA.id}/drills/${invalidDrillId}`;
  await registerDynamicFirestoreRoot(drillPath, 'practice-plan-valid-drill');
  await registerDynamicFirestoreRoot(invalidDrillPath, 'practice-plan-invalid-drill');
  await withEmulatorAuthAdmin(async (_authAdmin, firestoreAdmin) => {
    await firestoreAdmin.doc(drillPath).set({ title: drillTitle, description: 'Exact run-owned drill block', estimatedTime: '10 min', createdAt: new Date().toISOString(), order: 1, fixtureRunId: FIXTURES.runId, scenarioId: activeCertificationScenario });
    await firestoreAdmin.doc(invalidDrillPath).set({ title: invalidDrillTitle, description: 'Invalid duration contract fixture', estimatedTime: '-2 min', createdAt: new Date().toISOString(), order: 2, fixtureRunId: FIXTURES.runId, scenarioId: activeCertificationScenario });
  });
  ...
  const ownerResult = JSON.parse(cli(ownerSession, ['run-code', `async page => {
    ...
    try {
      ...
      const closePlanner = async current => {
        if (await current.isVisible().catch(() => false)) await page.keyboard.press('Escape');
        await current.waitFor({ state: 'hidden', timeout: 10000 }).catch(() => {});
      };
      let dialog = await openPlanner();
      await dialog.getByRole('button', { name: 'Secure Protocol', exact: true }).click();
      const blankRejected = await page.getByText('Invalid Protocol', { exact: true }).count();
      ...
      await dialog.getByPlaceholder('e.g. Infield Foundations & Double Plays').fill('No blocks');
      await dialog.getByRole('button', { name: 'Secure Protocol', exact: true }).click();
      const emptyBlocksRejected = await page.getByText('Invalid Protocol', { exact: true }).count();
      await page.getByText('Invalid Protocol', { exact: true }).last().waitFor({ state: 'hidden', timeout: 10000 });
      await closePlanner(dialog);
      dialog = await openPlanner();
      await dialog.getByPlaceholder('e.g. Infield Foundations & Double Plays').fill('x'.repeat(121));
      await dialog.getByText(${JSON.stringify(drillTitle)}, { exact: true }).click();
      await dialog.getByRole('button', { name: 'Secure Protocol', exact: true }).click();
      const oversizedRejected = await page.getByText('Invalid Protocol', { exact: true }).count();
      await page.getByText('Invalid Protocol', { exact: true }).last().waitFor({ state: 'hidden', timeout: 10000 });
      await closePlanner(dialog);
      dialog = await openPlanner();
      await dialog.getByPlaceholder('e.g. Infield Foundations & Double Plays').fill('Invalid duration');
      await dialog.getByText(${JSON.stringify(invalidDrillTitle)}, { exact: true }).click();
      await dialog.getByRole('button', { name: 'Secure Protocol', exact: true }).click();
      ...
      await dialog.getByPlaceholder('Describe the overall goal of this training block...').fill('Run-owned practice template');
      await dialog.getByRole('button', { name: 'Secure Protocol', exact: true }).click();
      ...
      dialog = page.getByRole('dialog', { name: 'Optimize Protocol' });
      await dialog.getByPlaceholder('e.g. Infield Foundations & Double Plays').fill(${JSON.stringify(editedMarker)});
      await dialog.getByRole('button', { name: 'Secure Protocol', exact: true }).click();
  ...
  expectEqual(ownerResult.blankRejected > 0 && ownerResult.emptyBlocksRejected > 0 && ownerResult.oversizedRejected > 0 && ownerResult.durationRejected > 0, true, 'Practice template validation rejects blank title empty blocks invalid durations and oversized text');

  const template = await withEmulatorAuthAdmin(async (_authAdmin, firestoreAdmin) => {
    const snapshot = await firestoreAdmin.collection(`teams/${teamA.id}/practice_templates`).where('title', '==', editedMarker).get();
    if (snapshot.size !== 1) throw new Error(`Expected one run-owned practice template, received ${snapshot.size}.`);
...
async function runPracticeDrillWorkflowAudit() {
  ...
  browserSelectScheduleTeam(ownerSession, teamA.id);
  const ownerResult = JSON.parse(cli(ownerSession, ['run-code', `async page => {
    const observer=(${createPracticeBrowserObserver.toString()})(page,{baseUrl:${JSON.stringify(BASE_URL)},prefix:'drill'});
    observer.start(['drill-link-invalid','drill-duplicate-empty']);
    ...
    try {
      ...
      observer.start(['drill-crud','drill-persistence']);
      await page.getByRole('button', { name: ${JSON.stringify(`Edit ${marker}`)} }).click();
      ...
      observer.start(['drill-duplicate-empty']);
      await page.getByRole('button', { name: 'Publish Drill', exact: true }).click();
      dialog = page.getByRole('dialog', { name: 'Publish Drill' });
      ...
      observer.start(['drill-responsive']);
      const practiceMeasurements=[],drillMeasurements=[];
      for(const viewport of [{width:1440,height:900},{width:390,height:844}]){
        await page.setViewportSize(viewport);await page.goto(${JSON.stringify(`${BASE_URL}/practice`)});
        await (${dismissFilmTeamAlert.toString()})(page);
      ...
      return { emptyDisabled, invalidUrlRejected, duplicateRejected, createdAfterReload, editedAfterReload, exactCount, partialCount, specialCount, emptyCount, renderedOrder, practiceMeasurements, drillMeasurements, ...observer.finish() };
  ...
  const memberResult = JSON.parse(cli(memberSession, ['run-code', `async page => {
    ...
    try{
      ...
      for(const viewport of [{width:1440,height:900},{width:390,height:844}]){
        ...
        practiceMeasurements.push(await (${measurePracticeBounds.toString()})(page,{heading,playbook}));
        await page.goto(${JSON.stringify(`${BASE_URL}/drills`)});await (${dismissFilmTeamAlert.toString()})(page);
        const drill=page.getByRole('heading',{name:${JSON.stringify(editedMarker)},exact:true,level:3});
        await drill.waitFor({timeout:15000});
        drillMeasurements.push(await (${measurePracticeBounds.toString()})(page,{drill,search:page.getByPlaceholder('Search drills...')}));
      }
      return{visible:await page.getByRole('heading',{name:${JSON.stringify(editedMarker)},exact:true,level:3}).count(),edit:await page.getByRole('button',{name:${JSON.stringify(`Edit ${editedMarker}`)},exact:true}).count(),practiceMeasurements,drillMeasurements,...observer.finish()};
    }finally{observer.finish();}
  }`]));
  expectEqual(memberResult.visible === 1 && memberResult.edit === 0, true, 'Practice member sees drill without mutation controls');
  const measurements={ownerPractice:ownerResult.practiceMeasurements,ownerDrills:ownerResult.drillMeasurements,memberPractice:memberResult.practiceMeasurements,memberDrills:memberResult.drillMeasurements};
  expectEqual(Object.values(measurements).every(validatePracticeBounds),true,'Practice and drills surfaces fit desktop and mobile viewports '+JSON.stringify(measurements));
  ...
  const deleteResult=JSON.parse(cli(ownerSession, ['run-code', `async page => {
    const observer=(${createPracticeBrowserObserver.toString()})(page,{baseUrl:${JSON.stringify(BASE_URL)},prefix:'drill'});
    observer.start(['drill-crud']);
    try{
  ...
  expectEqual([ownerResult,memberResult,deleteResult].flatMap(result=>result.consoleErrors).length,0,'Practice drill workflow console errors');
  expectEqual([ownerResult,memberResult,deleteResult].flatMap(result=>result.failedResponses).length,0,'Practice drill workflow failed responses');
  for(const [alias,result] of [['qa-coach-owner-a',ownerResult],['qa-team-member',memberResult],['qa-coach-owner-a',deleteResult]]){
    for(const caseId of [...new Set(result.observedResponses.map(response=>response.tag))]){
      requirePracticeResponses(result.observedResponses,caseId,{mutation:['drill-crud','drill-reorder','drill-link-valid'].includes(caseId)});
      await captureBrowserOperationRequests(caseId,alias,result.observedResponses,caseId);
    }
  }
}
...
async function runMediaWorkflowAudit() {
  ...
  let stepNumber=0;
  const browserStep=async(session,actor,cases,body,{nativeConfirm=false}={})=>{
    let raw=cli(session,['run-code',`async page=>{page.mediaStepPromise=(async()=>{const observer=(${createMediaBrowserObserver.toString()})(page,{baseUrl:${JSON.stringify(BASE_URL)}}),dismiss=${dismissFilmTeamAlert.toString()},measure=${measurePracticeBounds.toString()};observer.start(${JSON.stringify(cases)});let value;try{value=await(async()=>{${body}})();}finally{var observation=observer.finish();}return{value,...observation};})();return await page.mediaStepPromise;}`]);
    const stem=`media-step-${++stepNumber}-${cases[0]}`;writeFileSync(path.join(certificationArtifactDir,stem+'-stdout.txt'),redact(raw));
    if(nativeConfirm&&raw.trim()===''){cli(session,['dialog-accept']);raw=cli(session,['run-code','async page=>{const result=await page.mediaStepPromise;delete page.mediaStepPromise;return result;}']);writeFileSync(path.join(certificationArtifactDir,stem+'-resumed-stdout.txt'),redact(raw));}
    const result=parseMediaBrowserEnvelope(raw,cases[0]);
  ...
  const athleteUrl=`${BASE_URL}/roster`;
  const openOwnProfile=`await page.goto(${JSON.stringify(athleteUrl)});await page.getByText(${JSON.stringify(memberName)},{exact:true}).first().waitFor({timeout:15000});await dismiss(page);await page.getByText(${JSON.stringify(memberName)},{exact:true}).first().click();const dialog=page.getByRole('dialog',{name:${JSON.stringify('Player Profile: '+memberName)},exact:true});await dialog.waitFor({timeout:15000});await dialog.getByRole('region',{name:'My player media',exact:true}).waitFor({timeout:15000});`;
  await browserStep(adultSession,'qa-adult-player-a',['media-player-self'],`${openOwnProfile}const pending=page.waitForResponse(response=>response.url().startsWith(${JSON.stringify(BASE_URL+'/api/media?')})&&response.request().method()==='POST',{timeout:15000});await dialog.getByLabel('Upload player avatar',{exact:true}).setInputFiles(${JSON.stringify(imageFile)});if((await pending).status()!==201)throw Error('Player avatar upload failed');await dialog.getByRole('status').filter({hasText:'Player avatar saved'}).waitFor({timeout:15000});await page.reload();await dismiss(page);return true;`);
  ...
  await browserStep(adultSession,'qa-adult-player-a',['media-player-self'],`${openOwnProfile}await dialog.getByAltText('My player avatar',{exact:true}).waitFor({timeout:15000});const pending=page.waitForResponse(response=>response.url().startsWith(${JSON.stringify(BASE_URL+'/api/media?')})&&response.request().method()==='POST',{timeout:15000});await dialog.getByLabel('Upload gallery photo',{exact:true}).setInputFiles(${JSON.stringify(imageFile)});if((await pending).status()!==201)throw Error('Gallery upload failed');await dialog.getByRole('status').filter({hasText:'Gallery photo saved'}).waitFor({timeout:15000});await dialog.getByAltText('My gallery photo 1',{exact:true}).waitFor({timeout:15000});await page.reload();await dismiss(page);return true;`);
  ...
  for(const [session,actor,route]of[[owner,'qa-coach-owner-a','/settings'],[adultSession,'qa-adult-player-a',athleteUrl.slice(BASE_URL.length)]]){
    const rows=await browserStep(session,actor,['media-responsive'],`const rows=[];for(const viewport of[{width:1440,height:900},{width:390,height:844}]){await page.setViewportSize(viewport);${actor==='qa-coach-owner-a'?`await page.goto(${JSON.stringify(BASE_URL)}+${JSON.stringify(route)});await dismiss(page);const control=page.getByRole('button',{name:'Change profile photo',exact:true});await control.waitFor({timeout:15000});rows.push(await measure(page,{upload:control}));`:`${openOwnProfile}const panel=dialog.getByRole('region',{name:'My player media',exact:true});await panel.scrollIntoViewIfNeeded();rows.push(await measure(page,{dialog,avatarUpload:panel.getByLabel('Upload player avatar',{exact:true}),galleryUpload:panel.getByLabel('Upload gallery photo',{exact:true}),avatarDelete:panel.getByRole('button',{name:'Delete player avatar',exact:true}),galleryDelete:panel.getByRole('button',{name:'Delete gallery photo 1',exact:true}),status:panel.getByRole('status')}));`}await page.screenshot({path:${JSON.stringify(path.join(certificationArtifactDir,'media-'+actor))}+'-'+viewport.width+'.png',fullPage:false});}return rows;`);
  ...
  await browserStep(adultSession,'qa-adult-player-a',['media-delete'],`${openOwnProfile}for(const [button,status]of[['Delete gallery photo 1','Gallery photo deleted'],['Delete player avatar','Player avatar deleted']]){const pending=page.waitForResponse(response=>response.url().startsWith(${JSON.stringify(BASE_URL+'/api/media?')})&&response.request().method()==='DELETE',{timeout:15000});await dialog.getByRole('button',{name:button,exact:true}).click();if((await pending).status()!==200)throw Error('Own image delete failed');await dialog.getByRole('status').filter({hasText:status}).waitFor({timeout:15000});}await page.reload();await dismiss(page);return true;`);
  check('media-delete',await exists(galleryPath),false,'visible gallery deletion removes exact object');check('media-delete',await exists(playerAvatar),false,'visible avatar deletion removes exact object');check('media-delete',await withEmulatorAuthAdmin(async(_auth,db)=>{const data=(await db.doc(`players/${adult}/recruitingProfile/profile`).get()).data();return data.photos.length===0&&!data.photoURL;}),true,'own avatar and gallery metadata removed after reload');check('media-delete',(await request('media-delete','qa-adult-player-a',galleryPath)).status,404,'prior gallery URL revoked');check('media-delete',(await request('media-delete','qa-adult-player-a',playerAvatar)).status,404,'prior avatar URL revoked');
  const teammate=FIXTURES.firestoreDocuments.find(item=>item.path.startsWith(`teams/${team.id}/members/`)&&item.data.playerId===youth);
...
function browserCreateChatChannel(session, { teamId, memberName, channelName }) {
  return JSON.parse(cli(session, ['run-code', `async page => {
    ...
    try{
      ...
      return{chatId:body.chatId,status:response.status(),observedResponses,consoleErrors,failedResponses};
...
function browserChatSend(session, { teamId, chatId, marker, tag = 'chat-sync', pageProperty = '' }) {
  return JSON.parse(cli(session, ['run-code', `async page => {
    ...
    try{await page.goto(${JSON.stringify(`${BASE_URL}/chats/${chatId}?teamId=${teamId}`)});const input=page.getByPlaceholder('Tactical update...');await input.waitFor({timeout:15000});
      ...
      const pending=page.waitForResponse(response=>response.url()===${JSON.stringify(`${BASE_URL}/api/teams/chat/message`)}&&response.request().method()==='POST',{timeout:15000});
      await page.getByRole('button',{name:'Send message'}).click();const response=await pending;await page.getByText(${JSON.stringify(marker)},{exact:true}).waitFor({timeout:15000});
      return{status:response.status(),emptyDisabled,count:await page.getByText(${JSON.stringify(marker)},{exact:true}).count(),observedResponses,consoleErrors,failedResponses};
    }finally{page.off('console',onConsole);page.off('pageerror',onError);page.off('response',onResponse);}
  }`]));
}
...
function browserChatOfflineRetry(session, { teamId, chatId, marker, pageProperty = '' }) {
  return JSON.parse(cli(session, ['run-code', `async page => {
    ...
    try{await page.goto(${JSON.stringify(`${BASE_URL}/chats/${chatId}?teamId=${teamId}`)});const input=page.getByPlaceholder('Tactical update...');await input.waitFor({timeout:15000});await input.fill(${JSON.stringify(marker)});
      ...
      await page.context().setOffline(false);await page.waitForFunction(()=>navigator.onLine===true);
      const pending=page.waitForResponse(response=>response.url()===${JSON.stringify(`${BASE_URL}/api/teams/chat/message`)}&&response.request().method()==='POST',{timeout:15000});await page.getByRole('button',{name:'Retry send',exact:true}).click();const response=await pending;await page.getByText(${JSON.stringify(marker)},{exact:true}).waitFor({timeout:15000});
      return{retained,requestIds,status:response.status(),count:await page.getByText(${JSON.stringify(marker)},{exact:true}).count(),observedResponses};
    }finally{await page.context().setOffline(false).catch(()=>{});page.off('request',onRequest);page.off('response',onResponse);}
  }`]));
}
...
function browserChatQualityEvidence(session, { teamId, chatId, actor, pageProperty = '' }) {
  return JSON.parse(cli(session, ['run-code', `async page => {
    ${pageProperty ? `page=page[${JSON.stringify(pageProperty)}];if(!page)throw Error(${JSON.stringify(`Missing peer page ${pageProperty}.`)});` : ''}
    const observedResponses=[],consoleErrors=[],failedResponses=[],bounds=[],dismissedPriorityAlerts=[];let tag='chat-console';
    const onConsole=message=>{if(message.type()==='error')consoleErrors.push(message.text());};const onError=error=>consoleErrors.push(error.message);
    const onResponse=response=>{const url=response.url();if(!url.startsWith(${JSON.stringify(BASE_URL)}))return;const pathname=url.slice(${JSON.stringify(BASE_URL)}.length).split(/[?#]/,1)[0]||'/',status=response.status();if(status>=500)failedResponses.push({pathname,status});observedResponses.push({tag,pathname,method:response.request().method(),status,startedAt:new Date().toISOString(),completedAt:new Date().toISOString()});};
    page.on('console',onConsole);page.on('pageerror',onError);page.on('response',onResponse);
    try{for(const [caseId,viewport] of [['chat-console',{width:1440,height:900}],['chat-network',{width:1440,height:900}],['chat-responsive',{width:1440,height:900}],['chat-responsive',{width:390,height:844}]]){tag=caseId;await page.setViewportSize(viewport);await page.goto(${JSON.stringify(`${BASE_URL}/chats`)});await page.getByRole('heading',{name:'Coordination Hub',exact:true}).waitFor({timeout:15000});if(caseId==='chat-responsive'){for(let attempt=0;attempt<4;attempt++){const priorityAlert=page.getByRole('dialog',{name:'High Priority Team Alert'});const visible=await priorityAlert.waitFor({state:'visible',timeout:1800}).then(()=>true).catch(()=>false);if(!visible)break;const control=priorityAlert.getByRole('button',{name:'Got It',exact:true});if(await control.count()!==1)throw Error('High-priority team alert did not expose exactly one acknowledgement control during Chat responsive evidence.');await priorityAlert.getByRole('button',{name:'Got It',exact:true}).click();await priorityAlert.waitFor({state:'hidden',timeout:5000});dismissedPriorityAlerts.push({actor:${JSON.stringify(actor)},caseId,viewport});}if(await page.getByRole('dialog').count()!==0)throw Error('Unexpected dialog blocked Chat responsive evidence.');}const link=page.locator(${JSON.stringify(`a[href="/chats/${chatId}?teamId=${teamId}"]`)});await link.waitFor({timeout:15000});
        ...
        if(caseId==='chat-responsive'){await link.click();const input=page.getByPlaceholder('Tactical update...');await input.waitFor({timeout:15000});bounds.push({actor:${JSON.stringify(actor)},caseId,viewport,box:await input.boundingBox(),fits:await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)});}}
      return{observedResponses,consoleErrors,failedResponses,bounds,dismissedPriorityAlerts};
    }finally{page.off('console',onConsole);page.off('pageerror',onError);page.off('response',onResponse);}
  }`]));
}
...
async function runCommunicationWorkflowAudit() {
  const team=FIXTURES.teams.find(item=>item.alias==='qa-team-a'),teamB=FIXTURES.teams.find(item=>item.alias==='qa-team-b');
  const memberAlias='qa-team-member',ownerAlias='qa-coach-owner-a',memberUid=identityByAlias.get(memberAlias).uid,ownerUid=identityByAlias.get(ownerAlias).uid;
  const marker=`QA Chat ${certificationRunId}`,channelName=`QA Channel ${certificationRunId}`,teamPath=`teams/${team.id}`;
  const originalTeam=await withEmulatorAuthAdmin(async(_auth,db)=>(await db.doc(teamPath).get()).data());
  registerFirestoreDocumentRestoration(teamPath,originalTeam,'chat-team-settings',activeOperationResourceRegistry);
  ...
  const aliases=[ownerAlias,memberAlias,'qa-parent-a','qa-adult-player-a','qa-removed-member','qa-coach-owner-b','qa-multi-org'];
  const tokens=new Map();for(const alias of aliases)tokens.set(alias,(await signIn(alias)).body.idToken);
  const multiOrgUid=identityByAlias.get('qa-multi-org').uid,legacyMultiMemberId='legacy-multi-org-member';
  await withEmulatorAuthAdmin(async(_auth,db)=>{
    const directMemberRef=db.doc(`teams/${teamB.id}/members/${multiOrgUid}`),legacyMemberRef=db.doc(`teams/${teamB.id}/members/${legacyMultiMemberId}`),legacyChatRef=db.doc(`teams/${teamB.id}/groupChats/qa-team-chat`);
    const [directMember,legacyChat]=await Promise.all([directMemberRef.get(),legacyChatRef.get()]);
    const memberData=directMember.data(),chatData=legacyChat.data();
    if(!directMember.exists||!legacyChat.exists)throw Error('Chat legacy directory fixture prerequisites are missing.');
    const legacyChatData=Object.fromEntries(Object.entries(chatData).filter(([key])=>!['isDeleted','memberAuthorities','teamId'].includes(key)));
    const batch=db.batch();batch.set(legacyMemberRef,{...memberData,id:legacyMultiMemberId});batch.delete(directMemberRef);batch.set(legacyChatRef,legacyChatData);await batch.commit();
  });
  ...
  const chatId=created.chatId,chatPath=`${teamPath}/groupChats/${chatId}`;registerDynamicFirestoreRoot(chatPath,`chat-${chatId}`);
  const read=()=>withEmulatorAuthAdmin(async(_auth,db)=>(await db.doc(chatPath).get()).data());
  const messages=()=>withEmulatorAuthAdmin(async(_auth,db)=>(await db.collection(`${chatPath}/messages`).get()).docs.map(doc=>doc.data()));
  ...
  await browserLoginPeerContext(owner, 'qa-multi-org', '/dashboard', 'qaChatMulti', ownerAlias);const multiResult=JSON.parse(cli(owner,['run-code',`async page=>{page=page.qaChatMulti;if(!page)throw Error('Missing peer page qaChatMulti.');const observedResponses=[],consoleErrors=[],failedResponses=[],permissionWarnings=[],chatDirectoryStatuses=[];let dismissedPriorityAlerts=0,dialogIdentity='',dialogControlCount=0;const tag='chat-team-b';const onConsole=message=>{const text=message.text();if(message.type()==='error')consoleErrors.push(text);if(text.includes('collectionGroup permission-denied')||text.includes('No matching allow statements'))permissionWarnings.push(text);};const onResponse=response=>{const url=response.url();if(!url.startsWith(${JSON.stringify(BASE_URL)}))return;const status=response.status(),pathname=url.slice(${JSON.stringify(BASE_URL)}.length).split(/[?#]/,1)[0]||'/';if(pathname==='/api/teams/chat'&&response.request().method()==='GET')chatDirectoryStatuses.push(status);if(status>=500)failedResponses.push({pathname,status});observedResponses.push({tag,pathname,method:response.request().method(),status,startedAt:new Date().toISOString(),completedAt:new Date().toISOString()});};page.on('console',onConsole);page.on('response',onResponse);try{const directoryPending=page.waitForResponse(response=>response.url().startsWith(${JSON.stringify(`${BASE_URL}/api/teams/chat?`)})&&response.request().method()==='GET',{timeout:15000});await page.goto(${JSON.stringify(`${BASE_URL}/chats`)});await page.getByRole('heading',{name:'Coordination Hub',exact:true}).waitFor({timeout:15000});const directoryResponse=await directoryPending,directoryPayload=await directoryResponse.json();const projectedChannels=Array.isArray(directoryPayload.channels)?directoryPayload.channels:[],projectedB=projectedChannels.filter(channel=>channel.teamId===${JSON.stringify(teamB.id)}&&channel.id==='qa-team-chat');const directoryProjection={bCount:projectedB.length,privateFieldCount:projectedChannels.filter(channel=>['memberIds','memberAuthorities','unreadBy'].some(field=>field in channel)).length};const pathname=await page.evaluate(()=>location.pathname);for(let attempt=0;attempt<4;attempt++){const priorityAlert=page.getByRole('dialog',{name:'High Priority Team Alert'});const visible=await priorityAlert.waitFor({state:'visible',timeout:1800}).then(()=>true).catch(()=>false);if(!visible)break;dialogIdentity='High Priority Team Alert';const control=priorityAlert.getByRole('button',{name:'Got It',exact:true});dialogControlCount=await control.count();if(dialogControlCount!==1)throw Error('High-priority team alert did not expose exactly one acknowledgement control.');await priorityAlert.getByRole('button',{name:'Got It',exact:true}).click();await priorityAlert.waitFor({state:'hidden',timeout:5000});dismissedPriorityAlerts+=1;}const a=page.locator('a[href="/chats/qa-team-chat?teamId=${team.id}"]'),b=page.locator('a[href="/chats/qa-team-chat?teamId=${teamB.id}"]');const directoryVisible=await Promise.all([a,b].map(link=>link.waitFor({state:'visible',timeout:15000}).then(()=>true).catch(()=>false)));if(!directoryVisible.every(Boolean))throw Error('Authorized multi-team chat directory did not render both exact channels.');const counts={a:await a.count(),b:await b.count()};const linkStates={aVisible:await a.isVisible(),bVisible:await b.isVisible()};await a.click();const aText=${JSON.stringify(`${team.visibleMarker} synthetic private message`)},bText=${JSON.stringify(`${teamB.visibleMarker} synthetic private message`)};const aVisible=await page.getByText(aText,{exact:true}).waitFor({timeout:15000}).then(()=>true).catch(()=>false);if(!aVisible)throw Error('Team A chat marker did not render for its authorized tenant.');const aMarker=await page.getByText(aText,{exact:true}).count(),aForeignMarker=await page.getByText(bText,{exact:true}).count();await page.goto(${JSON.stringify(`${BASE_URL}/chats`)});const bAgain=page.locator('a[href="/chats/qa-team-chat?teamId=${teamB.id}"]');const bLinkVisible=await bAgain.waitFor({state:'visible',timeout:15000}).then(()=>true).catch(()=>false);if(!bLinkVisible)throw Error('Team B chat channel did not remain visible after returning to the directory.');await bAgain.click();const bVisible=await page.getByText(bText,{exact:true}).waitFor({timeout:15000}).then(()=>true).catch(()=>false);if(!bVisible)throw Error('Team B chat marker did not render for its authorized tenant.');const bMarker=await page.getByText(bText,{exact:true}).count(),bForeignMarker=await page.getByText(aText,{exact:true}).count();return{...counts,aMarker,aForeignMarker,bMarker,bForeignMarker,pathname,linkStates,chatDirectoryStatuses,directoryProjection,dismissedPriorityAlerts,dialogIdentity,dialogControlCount,consoleErrors,failedResponses,permissionWarnings,observedResponses};}finally{page.off('console',onConsole);page.off('response',onResponse);}}` ]));expectEqual(multiResult.pathname,'/chats','Chat chat-team-b: multi-team directory stays on exact route');expectEqual(multiResult.chatDirectoryStatuses.includes(200),true,'Chat chat-team-b: authenticated server directory returns 200');expectEqual(JSON.stringify(multiResult.directoryProjection),JSON.stringify({bCount:1,privateFieldCount:0}),'Chat chat-team-b: legacy Team B appears once in a private-field-free server projection');expectEqual(JSON.stringify(multiResult.linkStates),JSON.stringify({aVisible:true,bVisible:true}),'Chat chat-team-b: exact Team A and Team B links are both visible');expectEqual(multiResult.dismissedPriorityAlerts===0||(multiResult.dialogIdentity==='High Priority Team Alert'&&multiResult.dialogControlCount===1),true,'Chat chat-team-b: only the exact high-priority alert acknowledgement can precede tenant navigation');expectEqual(JSON.stringify({a:multiResult.a,b:multiResult.b}),JSON.stringify({a:1,b:1}),'Chat chat-team-b: composite identity renders same chat ID once per exact tenant');expectEqual(JSON.stringify({aMarker:multiResult.aMarker,aForeignMarker:multiResult.aForeignMarker,bMarker:multiResult.bMarker,bForeignMarker:multiResult.bForeignMarker}),JSON.stringify({aMarker:1,aForeignMarker:0,bMarker:1,bForeignMarker:0}),'Chat chat-team-b: exact tenant-qualified links open only their own marker messages');const legacyBackfill=await withEmulatorAuthAdmin(async(_auth,db)=>(await db.doc(`teams/${teamB.id}/groupChats/qa-team-chat`).get()).data());expectEqual(legacyBackfill.isDeleted,false,'Chat chat-team-b: authorized legacy lifecycle flag backfilled');expectEqual(legacyBackfill.teamId===teamB.id,true,'Chat chat-team-b: authorized legacy path team backfilled');expectEqual(legacyBackfill.memberAuthorities?.[multiOrgUid]?.teamId===teamB.id,true,'Chat chat-team-b: authorized legacy tenant authority backfilled');expectEqual(legacyBackfill.memberAuthorities?.[multiOrgUid]?.memberId===legacyMultiMemberId,true,'Chat chat-team-b: non-UID legacy membership document bound exactly');expectEqual(multiResult.permissionWarnings.length,0,'Chat chat-team-b: authorized directory emits no swallowed permission denial');expectEqual(multiResult.consoleErrors.length,0,'Chat chat-team-b: multi-team navigation console clean');expectEqual(multiResult.failedResponses.length,0,'Chat chat-team-b: multi-team navigation has no 5xx');await captureBrowserOperationRequests('chat-team-b','qa-multi-org',multiResult.observedResponses,'chat-team-b');
