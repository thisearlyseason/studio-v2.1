// Serialized into the owned Playwright CLI VM: use no unsupported URL globals.
export function createFeedBrowserObserver(page, { baseUrl }) {
  let tags = [];
  const requests = new WeakMap();
  const observedResponses = [], consoleErrors = [], failedResponses = [];
  const onRequest = request => {
    const match = /^(https?:\/\/[^/]+)(\/[^?#]*)/.exec(request.url());
    if (!match || match[1] !== baseUrl || !['/feed','/api/teams/feed/action'].includes(match[2]) || !tags.length) return;
  ...
  const onResponse = response => {
    ...
    if (status >= 500) failedResponses.push({pathname:request.pathname,status});
    for (const tag of [...new Set([...request.tags,'feed-console','feed-network'])]) {
      observedResponses.push({tag,pathname:request.pathname,method:request.method,status,startedAt:request.startedAt,completedAt:new Date().toISOString()});
    }
  };
