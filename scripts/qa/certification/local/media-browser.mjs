...
export function createMediaBrowserObserver(page,{baseUrl}){
  ...
  const onRequest=request=>{const match=/^(https?:\/\/[^/]+)(\/[^?#]*)/.exec(request.url());if(match&&match[1]===baseUrl&&['/settings','/team','/coaches-corner','/api/media','/api/media/recruiting'].includes(match[2])&&tags.length)requests.set(request,{tags:[...tags],pathname:match[2],method:request.method(),startedAt:new Date().toISOString()});};
