...
exports.normalizeEventKind = normalizeEventKind;
exports.buildUpcomingEventMessage = buildUpcomingEventMessage;
exports.getZonedClock = getZonedClock;
exports.shouldSendSameDayReminder = shouldSendSameDayReminder;
exports.candidateDateKeys = candidateDateKeys;
...
function normalizeEventKind(event) {
    ...
    if (raw.includes("game") || raw.includes("match"))
        return "game";
    if (raw.includes("practice") || raw.includes("training"))
        return "practice";
    if (raw.includes("tournament"))
        return "tournament";
    if (raw.includes("meeting"))
        return "meeting";
    if (raw.includes("tryout"))
        return "tryout";
    const title = String(event.title || "").toLowerCase();
    if (title.includes("game") || title.includes("match") || title.includes(" vs "))
        return "game";
...
function buildUpcomingEventMessage(event) {
...
function getZonedClock(now, requestedTimeZone) {
...
function shouldSendSameDayReminder(event, now, timeZone) {
...
function candidateDateKeys(now) {
    return [-1, 0, 1].map((offset) => {
        const date = new Date(now.getTime() + (offset * 24 * 60 * 60 * 1000));
        return date.toISOString().slice(0, 10);
    });
}
//# sourceMappingURL=event-reminders.js.map