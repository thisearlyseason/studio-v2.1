...
exports.buildCalendarFeed = buildCalendarFeed;
...
function buildCalendarFeed(events, teams, calendarName, generatedAt = new Date()) {
    ...
    for (const event of events) {
        ...
        lines.push("BEGIN:VEVENT", foldLine(`UID:${uid}`), `DTSTAMP:${stamp}`, `LAST-MODIFIED:${updatedAt ? utcValue(updatedAt) : stamp}`, `SEQUENCE:${sequence}`, `DTSTART;TZID=${timeZone}:${localValue(start)}`, `DTEND;TZID=${timeZone}:${localValue(end)}`, foldLine(`SUMMARY:${escapeText(prefix + title)}`), foldLine(`LOCATION:${escapeText(asText(event.location))}`), foldLine(`DESCRIPTION:${escapeText(description)}`), "STATUS:CONFIRMED", "TRANSP:OPAQUE", "END:VEVENT");
    }
    lines.push("END:VCALENDAR");
    return lines.join("\r\n");
}
//# sourceMappingURL=calendar-feed.js.map