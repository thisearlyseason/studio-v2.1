...
test('organization squad seats are explicit, capacity-bound, and organizer-controlled', async () => {
  ...
  assert.match(route, /buildOrganizationAssociationFields\(organization\)/);
  ...
  assert.match(hub, /if \(!firebaseAuth \|\| !organizationOwnerId\) return/);
  assert.doesNotMatch(hub, /if \([^)]*isHubDataLoading[^)]*\) return/);
...
test('chat unread state is recipient-specific, persisted by the server, and cleared only by an authorized reader', async () => {
  ...
  assert.match(message, /const memberships = await Promise\.all\(recipientIds\.map/);
  assert.match(message, /for \(const recipientId of recipientUserIds\)/);
  assert.match(message, /transaction\.update\(chatRef, unreadUpdate\)/);
  assert.match(chat, /export async function PATCH/);
  assert.match(chat, /team-chat-\$\{action\}/);
  assert.match(chat, /\[`unreadBy\.\$\{auth\.uid\}`\]: 0/);
  assert.match(chat, /transaction\.update\(chatRef, \{ \[`unreadBy\.\$\{auth\.uid\}`\]: 0/);
  assert.match(chat, /collectionGroup\('groupChats'\)/);
  assert.match(chat, /'Cache-Control': 'private, no-store'/);
  assert.match(list, /setAuthorizedChats\(Array\.isArray\(payload\.channels\)/);
  assert.doesNotMatch(list, /collectionGroup\(db, 'groupChats'\)/);
