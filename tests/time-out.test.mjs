...
const { DIFFICULTY, STORAGE, firstToFive, volleyballPoint, baseballContact, normalizeSport, normalizeDifficulty, storageGet, storageSet } = core;
...
test('Time Out rejects corrupted stored sport and difficulty values',()=>{assert.equal(normalizeSport('curling'),'soccer');assert.equal(normalizeSport('hockey'),'hockey');assert.equal(normalizeDifficulty('impossible'),'easy');assert.equal(normalizeDifficulty('hard'),'hard')});
