...
export function communicationDb(initial) {
  ...
  class Query {
    ...
    where(field,operator,value) {if(operator!=='==') throw Error('Unsupported test query');return new Query(this.path,[...this.filters,[field,value]],this.group);}
    limit() {return this;}
    orderBy() {return this;}
    async get() {
      const docs=[...records].filter(([path,value])=>(this.group ? path.split('/').at(-2)===this.path : path.startsWith(`${this.path}/`)&&path.split('/').length===this.path.split('/').length+1)&&this.filters.every(([field,want])=>value[field]===want)).map(([path])=>snapshot(new Ref(path)));
      return {docs,size:docs.length,empty:docs.length===0};
    }
  }
  const db={collection:path=>new Query(path),doc:path=>new Ref(path),collectionGroup:path=>new Query(path,[],true),async runTransaction(work) {
    const pending=[];
    const result=await work({get:ref=>ref.get(),update:(ref,value)=>pending.push(()=>records.set(ref.path,{...records.get(ref.path),...structuredClone(value)})),create:(ref,value)=>pending.push(()=>ref.create(value)),set:(ref,value)=>pending.push(()=>ref.set(value)),delete:ref=>pending.push(()=>ref.delete())});
