...
export const SCORE_SHEET_RESOURCES: Resource[] = catalog.map(sheet => ({
  ...
  portraitDownloadUrl: `/downloads/score-sheets/the-squad-${sheet.id}-portrait.pdf`,
  ...
  content: {
    ...
    body: `## How to keep score\n\n${sheet.guide}\n\n## Download and print\n\nChoose the portrait or landscape PDF above and use the matching print orientation. Print on Letter paper at actual size, or fit to printable area on A4. Both packs include scoring grids and a guide. Copy log pages for additional entries and record the same game details on each continuation page.\n\n## Before the game\n\nConfirm local rules, period lengths, roster limits and tie-break format with your organizer. These original community worksheets are not governing-body official forms; use the required official form when mandated.\n\n## Scoring reference\n\n[View the governing scoring guidance](${sheet.source})`,
  },
}));
