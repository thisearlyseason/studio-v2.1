...
export async function POST(req:NextRequest){
  try{
    ...
    await adminDb.runTransaction(async transaction=>{
      ...
      if(targetKind==='tournament'&&scoringCodeHash){
        transaction.set(parentRef.collection('private').doc('scoring'),{teamId:targetId,eventId,scorekeeperCodeHash:scoringCodeHash,updatedAt:new Date().toISOString(),updatedBy:auth.uid});
        transaction.update(parentRef,{scoringCode:FieldValue.delete(),scoringCodeHash:FieldValue.delete()});
      }
    });
