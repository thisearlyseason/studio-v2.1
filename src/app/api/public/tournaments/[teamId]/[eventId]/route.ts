...
import { isActiveTournamentPortal, permitsLegacyOrPaidPortals, spectatorTournament } from '@/lib/public-portal-data';
...
export async function GET(
  ...
  try {
    ...
    const tournament = spectatorTournament(eventId, { ...data, teamId });
    if (!isActiveTournamentPortal(teamId, teamData, data) || !tournament.isActive) {
      return NextResponse.json({ error: 'Tournament not found.' }, { status: 404 });
    }
