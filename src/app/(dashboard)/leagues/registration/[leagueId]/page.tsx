...
export default function LeagueRegistrationAdminPage() {
  ...
  }, [db, leagueId, isAuthResolved, authUser?.uid]);
  ...
  const handleManualAdd = async () => {
    ...
    try {
      ...
      toast({ title: "Squad Enrolled", description: "Team has been injected into the league roster." });
    } catch (error) {
    ...
    <div className="space-y-8 pb-32 animate-in fade-in duration-700">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <Badge className="bg-primary text-white border-none font-black uppercase text-[9px] h-6 px-3 shadow-lg shadow-primary/20">Institutional recruitment</Badge>
            <h1 className="text-3xl md:text-4xl font-black uppercase tracking-tight mt-1">Recruit Pool</h1>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 px-2">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              {pipelineType === 'team' && (
                <Button className="rounded-xl h-11 px-6 font-black uppercase text-[10px] shadow-xl shadow-primary/20" onClick={() => setIsManualAddOpen(true)}>
          ...
          <Card className="rounded-[2.5rem] border-none shadow-xl overflow-hidden bg-white ring-1 ring-black/5">
            <CardContent className="p-0">
              <div className="overflow-x-auto">
                {isEntriesLoading ? (
                  <div className="py-32 text-center flex flex-col items-center gap-6"><Loader2 className="h-10 w-10 animate-spin text-primary" /><p className="text-xs font-black uppercase tracking-[0.3em]">Synchronizing Recruit Ledger...</p></div>
                ) : (
                  <table className="w-full text-left">
                    ...
                    <tbody className="divide-y divide-muted/50">
                        ...
                        <tr key={entry.id} className="hover:bg-primary/5 transition-colors group">
                          ...
                          <td className="px-10 py-6 text-right">
                            <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                              <Dialog>
                                <DialogTrigger asChild><Button variant="ghost" size="icon" className="h-10 w-10 rounded-xl border-2 bg-white hover:bg-primary hover:text-white transition-all"><Terminal className="h-5 w-5" /></Button></DialogTrigger>
                                <DialogContent className="rounded-[2.5rem] border-none shadow-2xl p-0 overflow-hidden max-w-2xl bg-white text-foreground">
                                  <div className="h-2 bg-primary w-full" /><div className="p-8 lg:p-10 space-y-8">
                                    <DialogHeader><DialogTitle className="text-3xl font-black uppercase tracking-tight">Institutional Audit</DialogTitle></DialogHeader>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                                      <div className="space-y-6">
                                        <p className="text-[10px] font-black uppercase text-primary tracking-[0.2em] px-1">Form Responses</p>
                                      ...
                                      <div className="space-y-8">
                                        <div className="space-y-3">
                                          <Label className="text-[10px] font-black uppercase tracking-widest ml-1">Personnel Dispatch</Label>
                                        ...
                                        <div className="bg-primary/5 p-6 rounded-[2rem] border-2 border-dashed border-primary/20 space-y-4"><div className="flex items-center gap-3"><Zap className="h-5 w-5 text-primary" /><p className="text-[10px] font-black uppercase text-primary tracking-widest">Coordinator Note</p></div><p className="text-[11px] font-medium leading-relaxed italic text-muted-foreground">Dispatching entries sends a secure alert to the squad coach for roster finalization. This operation is logged in the master compliance audit.</p></div>
                                      </div>
                                    </div>
                                    <DialogFooter className="pt-4"><DialogClose asChild><Button variant="outline" className="w-full h-14 rounded-xl border-2 font-black uppercase text-xs">Close Audit Report</Button></DialogClose></DialogFooter>
                                  </div>
                                </DialogContent>
                              </Dialog>
                      ...
                      {filteredEntries.length === 0 && !isEntriesLoading && (
                        <tr><td colSpan={3} className="py-32 text-center opacity-20 flex flex-col items-center gap-4"><History className="h-16 w-16 mx-auto" /><p className="text-sm font-black uppercase tracking-[0.3em]">Ledger Empty</p></td></tr>
                      )}
                    </tbody>
                  </table>
                )}
            ...
            <Card className="rounded-[3rem] border-none shadow-xl overflow-hidden bg-white ring-1 ring-black/5">
              ...
              <CardContent className="p-8 lg:p-10 space-y-10">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                  <div className="space-y-2"><Label className="text-[10px] font-black uppercase tracking-widest ml-1">Protocol Headline</Label><Input value={localConfig?.title || ''} onChange={e => handleUpdateConfig({ title: e.target.value })} className="h-14 rounded-2xl border-2 font-black text-lg shadow-inner focus:border-primary/20 transition-all" /></div>
      ...
      {/* Manual Roster Injection Dialog */}
          ...
          <div className="p-8 lg:p-10 space-y-8">
            <DialogHeader>
              <div className="flex items-center gap-4 mb-2">
                <div className="bg-primary/10 p-3 rounded-2xl text-primary"><UserPlus className="h-6 w-6" /></div>
                <DialogTitle className="text-2xl font-black uppercase tracking-tight">Manual Enrollment</DialogTitle>
              </div>
              <DialogDescription className="font-bold text-primary uppercase text-[10px] tracking-widest">Inject squad into institutional ledger</DialogDescription>
            </DialogHeader>
            <div className="space-y-5">
              ...
              <div className="space-y-2"><Label className="text-[10px] font-black uppercase tracking-widest ml-1">Coach Name</Label><Input placeholder="Full Legal Name" value={manualForm.coachName} onChange={e => setManualForm({...manualForm, coachName: e.target.value})} className="h-12 rounded-xl border-2 font-bold" /></div>
              <div className="space-y-2"><Label className="text-[10px] font-black uppercase tracking-widest ml-1">Contact Email</Label><Input type="email" placeholder="coach@organization.com" value={manualForm.email} onChange={e => setManualForm({...manualForm, email: e.target.value})} className="h-12 rounded-xl border-2 font-bold" /></div>
            </div>
            <DialogFooter>
              <Button className="w-full h-14 rounded-2xl text-lg font-black shadow-xl shadow-primary/20 active:scale-95 transition-all" onClick={handleManualAdd} disabled={isManualProcessing || !manualForm.teamName}>
