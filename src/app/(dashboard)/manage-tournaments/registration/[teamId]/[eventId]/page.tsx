              ...
              <CardContent className="p-0 divide-y">
                {((localConfig?.form_schema || config?.form_schema) || []).map((field, i) => (
                  <div key={field.id} className="p-8 flex items-center justify-between group hover:bg-muted/10 transition-colors">
                    ...
                    {i > 2 && (<Button variant="ghost" size="icon" className="h-10 w-10 rounded-xl text-destructive opacity-0 group-hover:opacity-100 transition-opacity" onClick={() => handleUpdateConfig({ form_schema: (localConfig?.form_schema || []).filter(f => f.id !== field.id) }, true)}><Trash2 className="h-5 w-5" /></Button>)}
                  </div>
                ))}
              </CardContent>
            </Card>
