...
Hosted **native-shell** coach/athlete sign-in, exact-account session reload, role-specific navigation, settings/logout, foreground re-verification and external-link guards now pass on iOS Simulator and Android Emulator. Android also passed actual emulator network-off/Retry/reconnect. The user approved a temporary protection exception for only the QA alias; the native bootstrap used no credential/cookie. The exception was removed after testing, so the QA host is protected again and correctly rejects credential-free native startup. Production website configuration is unchanged.

Live acceptance tests are opt-in: Android requires the `hostedQA=true` instrumentation argument, and iOS requires a disposable fixture input in the owned development app's cache. Both assert the exact QA origin and use the real hosted form handlers/backend. Never put credentials into source or a release artifact. See the acceptance record for result bundles and cleanup. iOS radio-off/reconnect and physical finger-scrolling remain device checks; native provider sign-in, push, signing and store submission remain unimplemented release work. This is not store certification.
