...
## Feed deletion and realtime logout closure — 2026-09-08

Focused production Feed testing exposed BUG-055: a successful post deletion could race a stale comment refresh and surface a 404 failure after the deleted card should have disappeared. The smallest safe repair made concurrent disappearance idempotent, hid the target optimistically, and prevented stale reads from restoring it. The exact production merge `a3312cb74106645b7c77eaeb74b3bed80f0c8973` created a new post and comment, deleted the post, reloaded, and observed it remain absent with only HTTP 200/201 Feed responses and zero console warnings/errors.

The subsequent real logout regression exposed BUG-056: shared Firestore listeners could outlive anonymous-demo revocation and emit permission failures. Feed polling and shared collection listeners now pause synchronously before cleanup, in-flight work is aborted, and late callbacks are ignored. A separate durable retry marker preserves ambiguous network failures without corrupting the original demo expiry timer; a definitive pre-mutation 403 resumes the still-valid session.
