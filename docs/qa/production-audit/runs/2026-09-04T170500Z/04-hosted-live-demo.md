...
## Repaired-candidate acceptance

- Commit: `e796fde6aafd478d8f25855ed10f7cdabad79c5d`
- Release gate: `33911442472` — PASS
- Staging workflow: `33911749713` — PASS
- App Hosting revision: `studio-build-2026-09-04-012`
- Fresh Squad Pro launch: PASS; `Apex Demo Squad` rendered
- Same-origin `POST /api/demo/exit`: HTTP 204
- Former `/dashboard` session: redirected to `/login?reason=expired`
- Auth lookup after cleanup: rejected because the anonymous identity no longer exists
- Firestore cleanup: HTTP 404 for the seeded user, team, representative event/chat/membership children, league, facility, and player documents

BUG-021 is resolved and the hosted live-demo publication blocker is closed. The broader production matrix remains blocked by the independent device, destructive-lifecycle, scheduler, rollback, and specialized-fixture cases named in `05-coverage-matrix.md`.
