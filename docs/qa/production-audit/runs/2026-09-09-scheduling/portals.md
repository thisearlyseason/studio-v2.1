...
## Second-review correction: dedicated coach waiver portal

The second review found a missing dedicated-portal case; the earlier broad portal conclusion did not cover it sufficiently. The public registration path above remained working. The separate authenticated `action=waiver` path rejected default-only terms (409), or archived custom terms without the required default. The dedicated page also had the duplicate consent-click loop and compared a local UI signature date with UTC today on the server.

- Default-only, combined standard/custom/library terms, explicit organizer default, and disabled-default behavior now have exact receipt and replay regressions. The first two failed before repair.
- The actual dedicated consent row failed its browser interaction before removing its redundant parent handler; dedicated and registration consent controls now pass desktop/mobile checks.
- A signer-timezone test failed with 400 for a valid local day different from UTC. The page now sends its timezone; the server validates today's date in that zone, rejects stale dates and invalid zones, and retains UTC compatibility for older requests. Authoritative `signedAt` and immutable receipt/replay protections are unchanged.
- Fresh local verification: 37 focused tests passed, no failures/skips; both consent browser checks passed; type checking, lint (zero errors, existing warnings), and production build passed. Independent review found no blockers.
- PR #70: https://github.com/thisearlyseason/studio-v2.1/pull/70. Code commit `d1ab5cb28caf3731e915a0c77182d2be93384397`.
- Live fixture preconditions verified: both default-only and combined configs require the default waiver while keeping `default_waiver_text` empty, matching the actual failure case. Public DTOs return 200. Fixtures belong only to anonymous QA owner `jxuQ8J086odQidf7AwsATbwMHFy1`.
- PR and branch release runs `34309973913` and `34309971027` passed every application, rules, dependency, and Functions check. Merged as `b07905a5fa738d224bc49299f09e2a60f9792b5a`.
- Exact production `b07905a5` verified on both aliases, deployment `dpl_8aKesYXWpU42JKxkVgZ4o7NpLNPs` Ready. Default-only desktop and combined mobile signatures returned 200 and displayed Verification Complete. Actual browser timezone America/Edmonton submitted September 8 while authoritative UTC timestamps were September 9; both succeeded.
- Combined mobile check: complete standard/custom/library text displayed; replay 200 with `replay=true`; stale hash 409; wrong code 403; missing authentication 401; no page errors or horizontal overflow. A diagnostic replay attempt initially copied HTTP/2 pseudoheaders and was rejected by the test client before transmission; retrying with only the required auth/content headers produced the recorded results.
- Read-only database inspection verified both immutable archived receipts contained the exact expected full terms, correct signer/local date, authoritative UTC timestamp, and matching receipt hash. All test-owned teams, profiles, archives, and registration-code mappings were removed; zero remained. Screenshot: `output/playwright/dedicated-waiver-mobile-success.png`.
- The same live page exposed a separate date-label defect: canonical October 12 displayed as October 11 in Edmonton. The page now reuses the existing local-calendar parser. A regression executes its actual JSX expression and proves canonical dates, legacy ISO instants, invalid dates and empty values. Red before repair, green afterward; 26 focused tests, type checking, lint (zero errors), production build and independent review pass. Only presentation changes; signing/API behavior is unchanged.
- Date-label PR #71 (https://github.com/thisearlyseason/studio-v2.1/pull/71) passed release runs `34310723342` and `34310726463`, merged as `6a9beb452bd1a7f66ef10adfced18cb36d562119`. Final production deployment `dpl_E64TWZBVWrqXuPdE262GzNZBCFg5` is Ready, with both `www.thesquad.pro` and `thesquad.pro` assigned. Live health returned that exact revision at 04:31:34 UTC. Local application/test/script source matches the merged candidate.
- Final deployed frontend acceptance used a controlled public-API response for the presentation-only date change, without creating additional production records. In America/Edmonton at widths 1440 and 390, October 12 rendered as Monday, Oct 12 (not October 11); standard waiver text remained present, no horizontal overflow or page errors, and the console had zero errors/warnings. Screenshot visually reviewed: `output/playwright/dedicated-waiver-date-final.png`. This render check is not additional backend persistence evidence; actual live signing/receipt/negative-case evidence is recorded above on PR #70, whose API behavior is unchanged by PR #71.
- Second-review scoped result: all four dedicated-portal defects are fixed and verified, with no remaining scoped functional blocker. All disposable live fixtures were removed, and the final render-only browser session was closed. Prior physical-device acceptance rows and the deferred cosmetic referee count remain unchanged; this is not a blanket full-SaaS certification.
