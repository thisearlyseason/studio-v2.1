...
| 4 | Owner, assistant, and ordinary-member permitted routes/controls | Owner and corrected Assistant Coach both reached `/coaches-corner`; final corrected ordinary-member rows reached `/roster` at both sizes, rendered eight same-origin `/icon.png` avatars, and had no error boundary, console/page error, same-origin request failure, or overflow | `PASS — BUG-008/009 RETIRED` |
...
| Application-console errors | The final corrected roster rows emitted `0` at both viewports. Earlier entries were classified as now-corrected fixture-only avatar resource noise, not application exceptions or handled authorization denials; see [04-roster-console-diagnostic.md](./04-roster-console-diagnostic.md). Explicit `403`/`500` probes elsewhere still emitted error-level entries; the corrected multi-org desktop row emitted `0` |
...
- Correction evidence: canonical member name, player ID, jersey, and same-origin `/icon.png` avatar fields produced `/roster` at both viewports with visible Roster headings and eight intact avatar image elements. Both final contexts recorded `/icon.png` image response `200`, `example.test` requests `0`, console/page errors `0`, same-origin request failures `0`, no error boundary, and overflow `0`. The former crash and later fixture-avatar noise did not recur.
