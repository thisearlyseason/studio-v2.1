...
| Description | A physical Android test initially received no notification after another member sent a live chat message. After the awaited-delivery repair, the device produced an audible notification and visible notification card. The installed icon remained visually inset, and the launcher did not show a notification dot despite an active card. |
...
| Reproduction consistency | User-observed the original missing delivery twice on a physical Android device. After deploying the awaited-delivery repair, the same device produced a beep and visible notification card for a cross-account chat while the message appeared in chat. The launcher dot remained absent and the installed icon appeared inset. |
| Root cause | The initial repair started active-member resolution and notification delivery in an unawaited promise after persisting the chat. App Hosting could complete the request and terminate that work before FCM or Web Push delivery. The icon manifest also declared the same artwork as `any maskable`, which can cause Android to add unwanted safe-area padding; the notification badge reused a full-color app icon. Android launcher dots remain launcher/channel-controlled when an active notification exists. |
| Fix | The chat route awaits member resolution and transport delivery before responding, preserves the successful chat write if a provider fails, and returns FCM/Web Push counts. The PWA now ships distinct versioned regular and maskable install icons, a dedicated monochrome notification badge, and service-worker cache v5. Existing sender exclusion, active-membership resolution, root worker registration, The Squad identity, and standards Web Push fallback remain in place. |
| Automated verification | Local authoritative `npm run verify` passed 414 application tests, 38 Firestore/Storage rules tests, typecheck, lint with zero errors, the app production build, and the Functions build. Protected staging workflow `33871399182` independently passed verification, configuration/ownership checks, Firestore indexes, Functions, rules, exact App Hosting rollout, and health for application commit `71bd8ca1e17662485cf791a74ae528cb387b62bb`. Fresh no-cache checks returned `status: ok` from revision `studio-build-2026-09-04-004`; the live manifest and worker reference the separate icons/badge and all four assets returned HTTP 200. |
| Remaining acceptance | Uninstall the old Android PWA/shortcut, reopen staging in Chrome, reinstall The Squad, and confirm the full adaptive launcher icon. Confirm notification-card click-through, sender exclusion, opt-out, denied permission, stale-token cleanup, and logout; repeat install and push acceptance on iPhone/iPad. A missing Android launcher dot alone is not an app delivery failure when the active notification card is present. |
| Status | PARTIAL DEVICE ACCEPTANCE — DELIVERY CONFIRMED; INSTALL/NEGATIVE CASES REMAIN |
