...
**Status:** Phase 2 findings followed up through 2026-09-04; twelve defects are resolved, BUG-011 is retired by product decision, and BUG-005 remains open after physical Android testing disproved two background-delivery candidates. Provider evidence and deterministic emulator evidence are recorded separately from the still-incomplete coverage matrix.
...
| Reproduction consistency | User-observed the original missing delivery twice on a physical Android device. After deploying the awaited-delivery repair, the same device produced a beep and visible card while the PWA was open, but no card after closing it. Reinstalling v2 artwork still showed a square image inside the launcher shape. The v3/current-project worker retest again had no closed-app card or dot and retained the inset icon while chat sync succeeded. |
| Root cause | The current-project worker repair was insufficient. Sanitized staging inspection found notifications enabled with three FCM records, two expired and one provider-valid, but zero standards Web Push registrations; the current-release chat request returned 200 without dormant Android delivery. The browser selected FCM first, so its PushSubscription lifecycle remained coupled to Firebase even though a standards worker/server path already existed. Separately, manifest v3 was full-frame but Next metadata and automatic/legacy favicon paths still advertised older inset artwork. |
| Fix | Commit `514f8fa1` makes standards Web Push the sole browser/PWA transport, automatically migrates already-enabled accounts, clears their legacy FCM records, replaces the old PushSubscription, uses one stable root worker, and extends delivery TTL to one hour. Versioned v4 art is now used by the manifest, notification card, Next automatic icon, Apple icon, metadata, legacy PNGs, and favicon; cache v7 forces refresh. Sender exclusion, active-membership resolution, click-through handling, and the monochrome notification badge remain. |
...
| Remaining acceptance | On Android, open staging once while online so automatic migration creates a `notificationWebPushSubscriptions` record and removes FCM records; then close it fully, send from the other account, confirm receipt, and tap into the correct chat. Uninstall/reinstall and confirm v4 launcher artwork fills its mask. Then confirm sender exclusion, opt-out, denied permission, stale-subscription cleanup, and logout; repeat install and push acceptance on iPhone/iPad. A missing Android launcher dot alone is not an app delivery failure when an active card is present. |
