...
| Automated verification | Local authoritative `npm run verify` passed: 393 application tests, 38 Firestore/Storage rules tests, app and Functions typechecks, lint with zero errors, and production builds. Protected staging workflow `33757773460` passed on `a66e8858`. Hosted manifest, worker, offline page, and both PNG icons returned HTTP 200; a real Chromium session reported a root-scoped active controlling worker and zero console errors/warnings. |
| Remaining acceptance | Deploy combined commit `d4d1b7ae`; remove the old Schedule shortcut; install The Squad on physical Android and iPhone/iPad Home Screen; enable Tactical Alerts; confirm receipt and click-through from another current channel member; confirm sender exclusion and opt-out. |
