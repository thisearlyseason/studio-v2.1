...
**Status:** Phase 2 findings followed up through 2026-09-04; twelve defects are resolved, BUG-011 is retired by product decision, and BUG-005 has partial physical Android acceptance after its awaited-delivery repair. Provider evidence and deterministic emulator evidence are recorded separately from the still-incomplete coverage matrix.
...
| Description | A physical Android test initially received no notification after another member sent a live chat message. After the awaited-delivery repair, the device produced an audible notification and visible notification card while the PWA remained open, but not when it was closed. The installed icon remained visually inset, and the launcher did not show a notification dot despite an active card. |
...
| Actual behavior | The original chat write completed without notification fan-out; the first repair could be terminated after its response. After awaiting delivery, foreground/open-PWA notification worked, but the closed PWA could retain a service worker initialized for the wrong Firebase project. The v2 icon separated manifest purposes but retained rounded white corners inside the actual pixel canvas. |
| Reproduction consistency | User-observed the original missing delivery twice on a physical Android device. After deploying the awaited-delivery repair, the same device produced a beep and visible card while the PWA was open, but no card after closing it. Reinstalling v2 artwork still showed a square image inside the launcher shape. |
| Root cause | The client push registrar returned any active root worker without verifying its Firebase configuration. On staging, that could preserve a worker initialized with the production fallback project: the open page received staging FCM, but the closed page depended on the mismatched worker. Firebase also requires custom click handling to be registered before its messaging libraries load. Separately, the v2 source PNGs contained rounded white corners, so declaring them maskable could not make the artwork full-bleed. Android launcher dots remain launcher/channel-controlled when an active notification exists. |
| Fix | The chat route awaits transport delivery. Worker registration now always requests the current environment-specific URL and refuses a stale active worker until the configured replacement activates; route-aware click handling is registered before Firebase imports. Full-bleed versioned v3 regular/maskable artwork removes the internal white canvas, and cache v6 forces refresh. The monochrome notification badge, sender exclusion, active-membership resolution, The Squad identity, and standards Web Push fallback remain. |
| Automated verification | Local authoritative `npm run verify` passed 417 application tests, 38 Firestore/Storage rules tests, typecheck, lint with zero errors, the app production build, and the Functions build. Protected staging workflow `33875194877` independently passed verification, configuration/ownership checks, indexes, Functions, rules, exact rollout, and health for commit `3592d71dacae816ae8edae19320a504d3de897b2`. No-cache health reported revision `studio-build-2026-09-04-005`; the v6 worker and v3 assets returned HTTP 200. A clean hosted Chromium session reported an activated controlling worker whose script URL contains the staging Firebase project and zero console errors or warnings. |
| Remaining acceptance | On Android, open the updated staging PWA once while online so v6 activates, close it fully, send from the other account, confirm receipt, and tap into the correct chat. Uninstall/reinstall and confirm the launcher artwork fills its mask. Then confirm sender exclusion, opt-out, denied permission, stale-token cleanup, and logout; repeat install and push acceptance on iPhone/iPad. A missing Android launcher dot alone is not an app delivery failure when an active card is present. |
| Status | EXACT SECOND REPAIR DEPLOYED — CLOSED-APP AND ICON DEVICE RETEST REQUIRED |
