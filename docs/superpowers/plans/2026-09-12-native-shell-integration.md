...
**PARTIAL, hosted access blocker resolved:** user-approved QA-only protection exception enabled actual iOS/Android hosted coach/athlete sign-in, exact-identity reload, navigation, settings/logout, foreground and external-link checks; all passed. Android emulator offline/reconnect/Retry passed. iOS real-radio offline/reconnect and finger-scrolling remain physical-device checks. The QA exception was restored afterward; no native automation secret, production change, provider/push or store-release claim. See the latest acceptance record for evidence and cleanup.
