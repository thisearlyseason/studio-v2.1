...
- No SaaS runtime change except the independently reviewed Task 3 hydration defect and the source-backed row-zero parent landing correction. The latter may change only the existing post-session, post-return-path login profile decision from `/dashboard` to `/family` for the exact known `parent` role; it must not weaken recorder or evidence validation. The bounded product regressions, evidence subsystem, tests/package entrypoint, approved Phase 9 evidence, and planning/report files are the only permitted scope.
...
The row-zero parent landing correction follows the validated `scenario-failed` / `scenario-action` / `window-validation` / `expectation-mismatch` terminal for `admission-route-qa-parent-a-mobile`. Source tracing establishes that login sent every non-admin profile to `/dashboard`, the mounted dashboard exposes the protected `Dashboard` heading, and a later layout effect redirects a parent to `/family`; the strict recorder correctly retains that transient protected render and the landing validator correctly rejects it. Add a real `LoginPage` effect RED that observes `/dashboard` instead of the required direct `/family` landing, preserve a canonical same-origin local return path and all superadmin/admin/member/missing-profile/setup admission branches, and change only the exact known-parent landing decision. Validate return paths identically at query ingestion and storage consumption; reject raw, encoded, and repeatedly encoded network paths, backslashes, controls, schemes, malformed URLs, and canonical/origin changes, while retaining valid local path/query/fragment navigation. Keep all Phase 9 recorder/window contracts unchanged. Run focused account/session/routing/evidence tests, native and simulated-Linux suites, repository verification, build/rules, and resource scans before independent review. Do not call providers, navigate hosted state, push, deploy, merge, or touch retained recovery artifacts during this correction.

The post-run row-zero diagnostic refinement keeps the hosted result closed and makes no retry or provider call. Instrument the real `validateActionWindow` export with a separate sanitized diagnostic sink and fixed schema/location/terminal/loading/page-error/console-error/request-failure/overflow/render-coherence/resource/policy checkpoint/reason pairs at the existing validation boundaries. Seed the internal schema pair before caller-input snapshot, capture snapshot and validation failure internally, and replay the fixed pairs afterward through a nonthrowing external reporter so graph-shape failures retain exact attribution while callback mutation, exceptions, and intrinsic tampering cannot affect validation. Preserve the exact validation order, predicates, 44-row plan, cleanup, product runtime, and evidence outputs; mirror the enum in guardian and terminal-certificate validation, rebuild and re-pin the deterministic child, and prove each boundary through focused RED/GREEN before the full offline gates.

The 2026-08-29 closed request-failure propagation amendment started at exact Task 2 commit `fc7d0c005b45fb29025d99dfa3976db12a3ac9c0`. Strict protocol RED used `node --test --test-name-pattern='request failure protocol|request failure certificate|diagnostic forgery' tests/phase9-browser-evidence.test.mjs` and failed `0/2` because neither the guardian terminal nor certificate diagnostic admitted the summary; the first implementation pass made that group `32/32`. The deterministic-input pin RED failed `0/1`, and `node --test --test-name-pattern='child runner build transform preserves' tests/phase9-browser-evidence.test.mjs` failed `0/1` at the unchanged `131072`-byte ceiling. The bounded Acorn transform now removes only generated `new Error(<literal>)` debug arguments, rejects an unenumerated error-message comparison, and preserves the one exact allowlisted control-flow message. Its focused GREEN was `1/1`, the final input-pin plus transform group was `2/2`, and the required request-failure/failure-terminal/certificate/child-runner GREEN was `96/96`. Candidate detail remains unauthoritative until complete clean child stdio closure; trailing output discards it as runner-invalid.

The pre-transform production bundle was `133516` bytes at SHA-256 `a8dc2a0bc767e6c819ba3f0ac67d9c865140085b43ff7d2bdfad387f51bc0607`. Two independent final builds were byte-identical at `122682` bytes and SHA-256 `394a6793bb56b773f8a604a13a1de46205492819db8ea5c36cd130c2ffd5ddea`, below the unchanged cap. Exact source pins are child source `99c402659e8a583a1a75e73bcacac4aa378965ed1bf9b97a945059bd65933518`, builder `c98e0b10ab7b3d9d5f1edacc6bae022068ca3796eb6d90df4888f325738e3a35`, Playwright client `43426642e29f5a13c10d4f9c1aa8afad669b015156ca2e3cc812c18ec7c1f219`, scenario contracts `9c5839d242de618b039607c2c75955eb9bd4a212fb6507314884d7def7890e38`, and scenarios `d71ac18c3012eeceb982a2ada523aef237049d40e1871cc4ad46cfdf2fca8d3f`; the actual JSON config bytes remained unchanged at `e825e374c830b947b3243dc9bcf09572d006a1ea57020899ae2e5f59d1648a3f`. Literal worktree pins and exact deployed Git-blob admission now cover every changed deterministic child input.

Fresh final local evidence passed complete Phase 9 `510/510`, fixture/identity/hygiene `204/204`, and `npm run verify` with Node `1139/1139`, components `71/71`, Firebase rules `38/38`, Next static generation `564/564`, and the Functions TypeScript build. Dry-run remained exact `44 = 40 + 4` with network, Firebase, and navigation false. The stripped-environment offline smoke used only `about:blank` and returned browsers `0`, network false, and Firebase false. Final committed browser-list, pathname, and exact marker scans found zero browser sessions, marked processes, transport materializations, disposable smoke/test/audit roots, or producer profiles. The same three pre-existing retained recovery workspaces had identical identity metadata before and after the gates; they were never opened or changed. The previously documented hosted rows-zero `scenario-failed` / `scenario-action` failed-certificate reference and every retained recovery artifact remain untouched. No hosted/provider/staging navigation, push, deployment, production action, or merge occurred.

The first cumulative-review correction at exact parent `3ec19162ecb1d01ee2dd4ec479aad01efef87834` addresses the Important finding that only admission-login forwarded the closed request-failure summary through the real production scenario exports. Direct route emitted only the coarse route-mismatch pair, while isolation, logout, fresh unauthenticated, pending-deletion stale/fresh, and active-baseline paths omitted the diagnostic callback. Strict RED against those exports failed all eight behavioral subtests plus callback isolation (`0/9`). Thread the callback through route-result, logout-stage, direct action-window, landing-window, and aggregation boundaries, but collect and replay diagnostics only after the validation decision so reporter mutation or exceptions cannot replace it. Preserve the exact five-field closed summary, fixed-pair-only malformed failures, and every non-request-failure absence rule. Existing child-close and terminal-certificate tests continue to prove clean-close propagation and trailing-output invalidation without adding a callback or test-only production surface.

The same scenario-export command then passed `9/9`; combined admission/scenario coverage passed `14/14`, broader protocol/certificate coverage passed `75/75`, and the required focused group passed `105/105`. Two final builds were byte-identical at `123470` bytes and SHA-256 `64959ad8e2517e7bbac069e17e6e4f9e1c18d233f31fb45322b81f95fdc00caf`, leaving `7602` bytes below the unchanged `131072` cap. Updated deterministic pins are scenario contracts `daff10166d35bde31459eb8b069d866417d2d7a05db96bce8592754a41c3f052` and scenarios `cd2e109a49acf1b1fda1721a2818eb6b495e762e1745b7393e16098cb1f265f5`; child source, builder, Playwright client, and runner-config bytes remain unchanged. Literal and exact Git-blob/worktree admission cover the rebuilt child and both changed deterministic inputs. Final local gates passed Phase 9 `519/519`, fixture/identity/hygiene `204/204`, and `npm run verify` with Node `1148/1148`, components `71/71`, Firebase rules `38/38`, Next production build, and Functions TypeScript build. Dry-run remained exact `44 = 40 + 4`; stripped offline smoke remained `about:blank` with zero browsers/network/Firebase. No retained recovery workspace or failed hosted certificate was inspected or mutated during this correction, and no hosted/provider/staging navigation, push, deployment, production action, or merge occurred.
...
The guarded lifecycle for deployed SHA `e943bf3ae27806cae97d6fa4066367aaa1b6784c` then stopped at row zero with `window-request-failure/request-failure-invalid` and the closed tuple `policy-blocked / other / other / subresource`; exact cleanup and independent `20/82/1 -> 0/0/0` absence were certified. Independent public Chrome reproduction identified the source as `net::ERR_BLOCKED_BY_ORB` on the cross-origin `picsum.photos` avatar fallback used when an authenticated profile has no avatar. Replace only those three missing-avatar fallbacks in `TeamProvider` with the same-origin `/icon.png` asset while preserving supplied avatars. Prove RED/GREEN through the production resolver, run the complete release gates and independent review, then deploy a new exact SHA before requesting one fresh guarded lifecycle authorization. Never retry the failed deployed SHA.
...
### Closed join-admin attribution and compressed child package

The guarded lifecycle at exact deployed application SHA `a6d4d4308fb2292f02574f6aecec99a6cec95d35` stopped safely at canonical context `admission-route-qa-no-team-mobile`, primary stage `scenario-action`, with the coarse `window-no-team-request/unscoped-staging-join-admin-api` result. Cleanup and independent `20/82/1 → 0/0/0` absence remained certified, no evidence was promoted, and no retry was made. The bounded correction derives one internal numeric rejection code for URL, method, resource type, body, frame, header schema, authorization, cookie, identifier, or incomplete recorder material; only the matching closed public reason is allowed through `window-no-team-join-admin`. Guardian and terminal-certificate schemas accept the same ten fixed pairs and no raw URL, headers, cookies, token, fixture identifier, provider value, or error text.

The additional closed attribution made the recovered deterministic child `131884` bytes, above the unchanged `131072` entrypoint cap. The builder now compiles and audits that recovered ESM independently, replaces its entry URL with a generated-only binding, compresses it using deterministic gzip, and emits a self-contained wrapper whose sole static import is `node:zlib`. The gzip header uses the fixed platform-independent OS value `255`, so Darwin- and Linux-side admission regenerate identical bytes. The wrapper binds the eval entry URL, imports recovered bytes only from an in-memory data URL, and removes the binding in `finally`; no generated pathname is created, reopened, or executed. Two final builds were byte-identical with wrapper `52855` bytes / SHA-256 `dfe5c8db973bc9face600471b7673fea6015f72221e4f32f200bec356e2f7fba`, recovered module `131884` bytes / SHA-256 `b5134d38b792bad9a01b3e718a668f814c02d20965519f340c7316321872ba22`, and gzip member `39348` bytes / SHA-256 `5c2b71680dbb82fc2c335197f7added07de0c1cfc5133a16470af74f007b2a91`. The recovered-module audit rejects every `import.meta` form, including computed and destructured access. The final builder input SHA-256 is `64538be543150abebad65d6ebd4dc8bdf5e1f82baee43b4a81185374a2236657`, Playwright client `ff50cc2d8db1a601f7398f687ca9785fd278876c65dc058cc2acb1c7c4904db1`, scenario contracts `4c4a869624923c0ef447843dc2c536051f98150ba380b9764e3cbdd78ed9ca16`, and unchanged child source `fa5666efef25296683091bf8e4b7d2d2a656b12a034cf87165dc46518f785a05`.

Strict RED/GREEN covered missing rejection attribution, incomplete capture, guardian/certificate symmetry, absent recovered/compiler/package exports, wrapper mutation, the real `--input-type=module --eval` boundary, and the still-oversized builder selection. The first complete Phase 9 run then correctly exposed an `invalidEvidenceCount` regression in the shared Firestore/Listen classifier plus two plaintext-child assertions (`545/559`); restoring the pre-existing counter and inspecting recovered bytes through the exact wrapper parser made all fourteen failed boundaries green. The first push Release gate then failed only on Ubuntu because Node's gzip header OS byte differed from the Darwin-built artifact (`3` versus `19`). A strict local RED observed Darwin `19` where the cross-platform contract required `255`; the builder now validates the other fixed header bytes and normalizes only the gzip OS field to the standards-defined platform-neutral value `255` before hashing, rendering, and regeneration comparison. Independent rereview returned GO with no findings and confirmed OS-specific and concatenated-member variants remain rejected. Fresh corrective evidence passed Phase 9 `562/562`, fixture/identity/hygiene `205/205`, and `npm run verify` through typecheck, lint with existing warnings only, the complete Node and component suites, Firebase rules, Next static generation `564/564`, and the Functions TypeScript build. Dry-run remained exact `44 = 40 + 4`; stripped-environment offline smoke used only `about:blank` and returned browsers `0`, network false, and Firebase false. Final scans found zero managed Phase 9 processes, transport materializations, disposable workspaces, or producer profiles. No hosted/provider/staging navigation, deployment, production action, or merge occurred during this correction.

The first independent review blocked release on three Important admission gaps. The recovered audit parsed with Acorn but used a lexical import scan and a substring entry-binding check; the package inspector accepted concatenated gzip members; and the recovered/gzip hashes were reported but not literal-pinned at runtime or deployed-Git-blob admission. Separate strict REDs reproduced side-effect data imports, dead string/comment binding text, an appended valid gzip member, and absent recovered/gzip pins. The corrected audit walks Acorn import declarations, re-export sources, import expressions, exact `globalThis.__phase9EntryUrl` member references, and every `import.meta` meta-property form; package inspection requires exact equality with a freshly generated deterministic gzip member; and CLI admission extracts the exact pinned wrapper payload, bounds decompression, and verifies literal gzip and recovered SHA-256 values for both the worktree runner and the admitted deployed Git blob. Focused review-fix coverage passed `48/48`; the final computed/destructured `import.meta` RED then passed after the complete meta-property rejection. Independent rereview returned GO with no Critical, Important, or Minor findings. Wrapper, recovered, and gzip bytes and hashes remained unchanged.
