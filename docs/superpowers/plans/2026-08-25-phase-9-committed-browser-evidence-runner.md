...
The definitive ownership amendment at exact parent `191ff1cc623581d3c415c3158898cecf871e5704` makes browser-inventory execution, parsing, schema, and type rejection enter the same irreversible inspection-uncertain state before recovery can retry. Replace the late aggregate ownership handoff with pinned protocol version 3: every new browser performs only exact local `open about:blank`, captures its frozen receipt, and synchronously emits a monotonic `ownership-add` before recorder, viewport, login, navigation, or scenario action; retained attachment emits `ownership-attach` before attachment action and binds the already frozen receipt. The guardian parses the bounded stream in real time, registers every acquisition immediately, freezes the set at `ownership-complete`, and rejects duplicates, mutation, sequence gaps, out-of-order records, or row claims for unowned sessions. Add first/late acquisition crash recovery, transient inventory-then-clean retry, malformed ownership, and exact `/tmp/phase9-core-identities.test` final removal regressions. Test cleanup may remove that one literal root only after browsers and marked processes close and exact non-symlink directory, mode `0700`, current ownership, and confinement checks succeed. Rebuild and repin deterministic child, client, and config artifacts, run all local gates, update the ignored report/progress chain, and do not perform hosted navigation or mutation, push, deploy, or merge.
