...
- [x] Close the final stat/delete boundary with transactional descriptor-relative quarantine/restore, and execute only captured helper source through the pinned isolated Python runtime despite path or startup-environment replacement.
...
- [x] **Step 5: Implement guardian validation and propagation** against the independently accepted monotonic stream. Preserve pending and active ownership for recovery, treat attribution as provisional until stdio is definitively closed and join proves no protocol failure, terminate and rejoin on an initial close timeout, retain attribution only across definitively closed clean-protocol closure failure, and keep malformed or trailing output classified as runner-invalid. Permit only `row-emission` after `ownership-complete` so fallible finalization cannot create a false success terminal.
...
- [x] **Step 3: Implement provider-free local finalization** with exact path/identity/mode/ownership and deterministic cleaned-journal checks, held-inode credential zeroization, retained in-place workspace quarantine, truthful retained-path/removal claims, terminal `failed` promotion, rows `0`, and idempotent replay.
- [x] **Step 4: Cover failure boundaries** for zeroization, every checkpoint/final promotion boundary, foreign child/workspace identity swaps, dirty journals, legacy migration, post-zeroization crash recovery, and terminal replay revalidation.
...
- [ ] **Step 6: After review only, execute the exact local command and verify the terminal failed certificate, zero-length exact credential inode, exact retained in-place workspace/manifest, and truthful non-removal disposition.** No provider call, hosted lifecycle, push, deployment, or merge.
