...
The sole failure terminal allowed after `ownership-complete` is `stage=row-emission`. This narrow exception covers fallible private-resource finalization and row serialization after the guardian has accepted the final ownership snapshot but before the success completion. Any other post-ownership stage, any trailing output after a failure terminal, or any duplicate terminal invalidates the candidate attribution as `scenario-runner-invalid`; attribution becomes authoritative only after child stdio is definitively closed with no protocol failure. A join timeout triggers exact termination and a second bounded join before attribution is selected. A definitively closed, clean-protocol child closure failure may retain the validated candidate while exact recovery remains guardian-owned; an unclosed stream never may.
...
The descriptor-rooted Playwright CLI deterministically writes sensitive ephemeral output beneath `.playwright-cli` in the owned `playwright-tmp` root. Profile cleanup admits that top-level directory only with the reviewed CLI filename grammar (`console-<UTC timestamp>.log` and `page-<UTC timestamp>.yml`), exact current-user ownership, `0700` directory and `0600` single-link regular-file modes, bounded depth/count/per-file bytes/total bytes, and no symlinks or other entry types. The cleanup holds and compares parent, root, directory, and file descriptors against named identities, then delegates removal to the pinned Darwin helper through inherited parent/root descriptors. The helper revalidates the complete admitted receipt inventory before any mutation and transactionally renames each exact entry to an unguessable descriptor-relative quarantine name before irreversible unlink/rmdir; a moved-identity mismatch is restored without deleting the foreign object, and pathname-recursive removal is forbidden. The helper source is captured and hashed before execution through the exact pinned, signed root-owned Python runtime with `-I`, a closed environment, and user-site/startup injection disabled, so later helper-path replacement cannot change executed bytes. Any foreign name, metadata mismatch, content-identity change, pathname replacement, inventory race, or partial helper failure withholds profile and fixture closure certification and retains the root or its exact recovery-owned partial state. Offline smoke uses the same cleanup contract and may invoke it only after a successful final close-and-empty-inventory proof; closure uncertainty preserves the entire workspace.
