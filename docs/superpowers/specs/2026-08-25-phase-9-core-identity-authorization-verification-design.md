...
`recover-terminal` is a local, recovery-only command. It admits only an identity-bound canonical result, the exact private workspace and its `manifest.json`/`credentials.json` children, an exact suffix-matched external quarantine path, and an exact v3 `cleaned` fixture journal whose deterministic graph and `20/82/1` counts match the certificate. It constructs no provider adapter and performs no seed, browser, transition, or evidence-write operation. Recovery never unlinks or recursively removes a credential or workspace pathname. After publishing a durable identity checkpoint, a pinned Darwin helper zeroizes the held original credential inode with `ftruncate`, data sync, full sync, and revalidation. A second durable checkpoint precedes an exclusive atomic rename of the held, revalidated workspace into the absent external quarantine, followed by parent-directory sync. The original workspace and credential paths are then proven absent, while the exact manifest and zero-length credential inode remain in the private retained quarantine for audit. The terminal certificate remains `failed`, browser rows remain `0`, and evidence remains `written=false`; recovery can never produce PASS.

Legacy `legacy-primary-unavailable` is permitted only when the recovery writer migrates a canonical version-1 checkpoint that contains no retained primary attribution. New hosted runs must provide primary attribution before every certificate write. Recovery certificate version 3 records only SHA-256 identity commitments plus the monotonic `validated`, `zeroized`, and `quarantined` disposition; it contains no raw path, UID, session, run identifier, or error. `credentialRemoved` and `workspaceRemoved` remain false because physical deletion is not claimed. Zeroization, quarantine, publication, identity, or replay uncertainty fails closed behind one sanitized recovery error and retains the recoverable original or quarantine state. Replay revalidates the result identity, committed identities, exact manifest, zero-length credential, original-path absence, and retained quarantine before returning the same terminal failure.
