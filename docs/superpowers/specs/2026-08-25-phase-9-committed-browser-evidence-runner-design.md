...
- derives target kind, parsed-evidence labels, and scope sets inside the installed request recorder. Each closed request/listener/response record is HMAC-bound to its recorder page, action-window sequence, channel, and complete fixed fields before CLI transport; the client verifies and strips that provenance before returning a sample. Unsigned, replayed, altered, raw, extra-field, disconnected, or target-incompatible records fail closed;
- never returns or logs credentials, cookies, bearer values, integrity keys/tags, request bodies, storage state, raw target URLs, raw run IDs, UIDs, team IDs, player IDs, or other raw fixture identifiers. Request bodies and exact probe targets are inspected only in private process memory, reduced to fixed labels/booleans/statuses, and immediately discarded. The complete sanitized sample, including `pageId` and every provider-controlled free-text field, receives a recursive fixture-literal check before public return.
...
- multiplexed Firestore bodies retain every valid parsed scope plus `unscoped` for malformed siblings, and unsigned/forged/disconnected/target-incompatible labels are rejected by a real recorder integration boundary;
